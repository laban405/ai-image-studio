...
export default function GenerativeFill() {
  ...
  const handleGenFill = async () => {
    ...
    console.log("handleGenFill 2", res);

    if (res?.data?.success) {
      console.log("handleGenFill 3");
    ...
    if (res?.data?.error) {
      console.log("error gen fill", res.data.error);
      setGenerating(false);
    }
    console.log("handleGenFill 4");
  };
